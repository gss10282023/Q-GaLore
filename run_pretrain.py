...
from q_galore_torch.utils.quantization import prepare_model_for_int8_training, QGaLoreLinear
from q_galore_torch.utils.simulate_quantization import prepare_model_for_int8_training_simulation, QLinear
...
def parse_args(args):
    ...
    parser.add_argument("--proj_bits", type=int, default=4)
    ...
    parser.add_argument("--cos_threshold", type=float, default=0.4)
    parser.add_argument("--gamma_proj", type=int, default=2)
    parser.add_argument("--queue_size", type=int, default=5)
    # Current real quantization implementation does not support DDP, using simulation mode for training with DDP
...
def main(args):
    ...
    if 'galore' in args.optimizer.lower() and not ('q_galore' in args.optimizer.lower()): 
    ...
    if args.optimizer.lower() == "adam":
        ...
        if args.simulation:
            ...
            optimizer = QGaLoreAdamW8bit_simulate(param_groups, lr=args.lr, weight_decay=args.weight_decay, betas=(0.9, args.beta2))
        else:
    ...
    # Layer-wise optimizers
        ...
        for p in model.parameters():
            if id(p) in id_galore_params:
                optimizer_dict[p] = QGaLoreAdamW8bit([{'params': [p], 'rank': args.rank, 'update_proj_gap': args.update_proj_gap * 2, 'scale': args.galore_scale, 'proj_type': args.proj_type,
                                                    "quant": args.proj_quant,'quant_n_bit': args.proj_bits, 'quant_group_size': args.proj_group_size,
